...
const APP_SECURE = process.env.NODE_ENV === "production";
...
/** ---------------- REDIS HELPERS ---------------- */
export async function setSessionDataInServer(sessionId: string, data: object) {
    await redis.set(
...
export async function getSessionDataInServer(sessionId: string) {
    const raw = await redis.get(`${SESSION_PREFIX}${sessionId}`);
    return raw ? JSON.parse(raw) : null;
}

export async function deleteSessionInServer(sessionId: string) {
