...
export abstract class FrameWorkAppService extends ShellEngine {
  ...
  private mergeTableConfig(
    overrides: DeepPartial<typeof DEFAULT_TABLE_CONFIG> = {},
  ): typeof DEFAULT_TABLE_CONFIG {
  ...
  protected BuildDetails(params: BuildDetailsInput): RenderResult {
    const { title, data, fields, tables = [], tabs } = params;
    const detailFields = mapFormFieldsToDetailFields(fields);
  ...
  protected async SaveMultipleData(params: {
    table: string;
    parentColumn: string;
    childColumn: string;
    parentId: number;
    ...
    const { table, parentColumn, childColumn, parentId, values } = params;

    try {
      await this.query(table).where(parentColumn, parentId).del();
