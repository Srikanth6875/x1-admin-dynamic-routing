...
import { TRIM_COLUMNS_CONFIG, TRIM_FIELDS, TRIM_TABLE_CONFIG, TRIM_TABLE_HEADING, TRIM_TABLE_ROW_ACTIONS } from "./Trim";
...
export class TrimAppService extends FrameWorkAppService {
  ...
  async TrimList() {
        ...
        return await this.BuildClarifyDataTable({
          ...
          row_actions: TRIM_TABLE_ROW_ACTIONS,
        });
  }

  async AddTrim(del: boolean = false): Promise<BuildFormResult> {
    const fields = TRIM_FIELDS();
    const make_options = await this.sql_query({ vm: TABLE_NAMES.VEHICLE_MAKE })
      .select("vm.id as value", "vm.make as label");

    const model_options = await this.sql_query({ vm: TABLE_NAMES.VEHICLE_MODEL })
