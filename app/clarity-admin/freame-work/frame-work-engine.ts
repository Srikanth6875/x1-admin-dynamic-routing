...
export abstract class ShellEngine {
    ...
    protected async executeRawQuery<T = any>(sql: string, bindings?: Knex.RawBinding[] | Record<string, any>): Promise<T[]> {
        const result = bindings ? await this.sql_query.raw(sql, bindings) : await this.sql_query.raw(sql);
